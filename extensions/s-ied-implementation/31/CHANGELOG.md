...
- Pending source and control reference matches now show their resolved vendor paths before you apply the link
- Data-model, Inputs and Outputs tables' "Unlink" button is now "Edit Link": it opens the ExtRef/ExtCtrl browser to change or unlink the mapping, instead of unlinking directly

### Removed

- Removed the "Status" column and its legend from the Inputs and Outputs tables

## [26] - 2026-08-12

### Added

- Data-model viewer now shows each data attribute's `valKind` with an explanation, including values inherited from the data type

## [25] - 2026-08-10

### Added

- Data-model comparison tree now shows a colored "Val" badge on any element that contains values in its subtree, summarizing the worst match status at a glance (shown regardless of collapsed/expanded state)

### Changed

- Data-model comparison table headers renamed from "Current"/"Incoming" to "Specification"/"Implementation" for clarity
- Element names in the data-model comparison tree now use a consistent grey text color instead of varying by element kind

## [24] - 2026-08-07

### Fixed

- Data-model comparison table now shows the project values for a data attribute, comparing it with the vendor's implemented value or the type default

## [23] - 2026-08-06

### Added

- Integrating an IED now records which ICD it came from (file name, uuid, version/revision) and the SCL version, revision and release of that ICD, so the project retains source provenance. Re-integrating the same ICD updates that entry, while a different ICD is kept as an additional entry in the import history

## [22] - 2026-07-28

### Changed

- Integrated vendor IEDs now keep the replaced S-IED name during apply, adding stable letter suffixes when one S-IED is split across multiple IEDs

## [21] - 2026-07-28

### Changed

- Sidebar now shows matching candidates as a grouped, expandable tree instead of separate cards, making it easier to scan many candidates
- Sidebar panel can be resized by dragging its edge
- Sidebar stays open after linking, so you can keep browsing without reopening it
- Sidebar now shows the "ICD Data Model" matching hint when matching uses LNode attributes (fallback when no LNodeSpecNaming is present)

### Fixed

- Mapping a data object to a vendor IED now only records where it's implemented (`mappedDoName`/`mappedDaName`) when the vendor names it differently than the specification; when the name matches, nothing extra is written. Re-applying to an already-mapped project also cleans up any leftover mapping info from before this fix

## [20] - 2026-07-23

### Fixed

- Mapping an LNode to a vendor IED now keeps the specification type on the LNode instead of overwriting it with the vendor type, so the specification type stays available to compare against the implemented IED type

## [19] - 2026-07-23

### Fixed

- Applying changes now keeps vendor-specific fields on the IED (e.g. Siemens identifiers and flags) instead of dropping them
- AccessPoints that only reference another server via ServerAt are no longer auto-connected during apply
- LN0s are now taken into the project when implementing an S-IED
