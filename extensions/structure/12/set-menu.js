import { T as k, ax as V, Z as B, a3 as S, Y as g, _ as L, ay as O, az as Y, aA as _ } from "./sdk-Cwwopi_5.js";
async function Z() {
  const r = localStorage.getItem("currentActiveFileDatabaseName");
  if (!r)
    throw new Error("no active file");
  const n = await K(), c = await k({ files: n });
  await q(r, c);
}
async function q(r, n) {
  ...
  await i.open();
  const m = L(i);
  await R(), await A(), await J(), i.close(), c.close();
  async function R() {
    ...
    for (const e of p) {
      const d = await E(i, e), y = [];
      for (const w of d)
        if (I(w, ["Substation", "VoltageLevel", "Bay"])) {
          const v = await x(c, w, "name");
          y.push(v);
        }
      m.instantiate(e);
      ...
      const D = {
        ...
        parent: null,
        children: []
      }, U = await t.addRecord(D);
      await t.ensureRelationship(f, U);
      const H = await m.findChildRecords(e);
      for (const w of H) {
        const v = await b(w);
      ...
      const N = await t.findRootSCL(), T = C.at(-1);
      N && T && await t.ensureRelationship(N, T);
    }
  }
  async function A() {
  ...
  async function F(a) {
    ...
    for (const e of o) {
      ...
      Y(d, "functionUuid", h?.value), await t.updateRecord(d);
    }
...
async function z(r, n, c) {
  ...
  const i = await r.db.table(S).where({ "parent.id": c.id }).filter(
    (R) => !!R.attributes?.some((A) => A.name === "type" && A.value === t.value)
  ).first();
...
async function E(r, n, c = []) {
  if (!n.parent)
    return [];
  const t = await G(r, n.parent.id);
  if (!t)
    throw console.error("could not find parent for", n), new Error("could not find parent for record");
  if (!t.parent)
    return [];
  const m = await E(r, t, c);
  return [t, ...m];
}
async function K() {
  const { promise: r, resolve: n } = Promise.withResolvers(), { open: c, onChange: t } = V({
    accept: "fsd"
  });
  return t((i) => {
