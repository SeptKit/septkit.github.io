import { T as H, ax as k, Z as B, a3 as S, Y as g, _ as L, ay as V, az as _ } from "./sdk-BSJ8JAgq.js";
async function Y() {
  const r = localStorage.getItem("currentActiveFileDatabaseName");
  if (!r)
    throw new Error("no active file");
  const n = await K(), c = await H({ files: n });
  await q(r, c);
}
async function q(r, n) {
  ...
  await i.open();
  const m = L(i);
  await R(), await A(), await x(), i.close(), c.close();
  async function R() {
