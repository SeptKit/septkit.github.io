...
- Instantiate a Function (FSD) or Application (ASD) via the merge review: right-click a node → Instantiate → Function/Application → pick a `.fsd` / `.asd` file → review and apply. The instantiated root Function is named `<App>_Fn` and its content is wired into the matching category `FunctionRole`(s) in the same operation.
