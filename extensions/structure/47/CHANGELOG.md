...
- ASD/FSD extraction migrated to `@dialecte/scl` transaction API :
  - `extractApplicationTo` removed from `scl-application` - superseded by Dialecte extension
  - `makeSubFunction`, `makeHeader`, `makeFunctionCategory` removed from SCL composite factory - no longer needed
  - `exportSclFile` replaces `exportFile` + `globalStore.downloadFile` pattern
