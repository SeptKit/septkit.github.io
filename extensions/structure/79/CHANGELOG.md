...
- Single LNodes under SubFunctions now show up to 20 characters of the LNode name; longer names are truncated and multiple LNodes still show the number count.

## [76] - 2026-08-18

### Added

- The side panel header shows the type of the selected element and a close button that hides the panel and clears the selection.

### Changed

- The side panel stays hidden until the information action of a card is used.
- The highlighted card marks the element the side panel is open for, instead of the last clicked element.
- The element name is shown as a read-only field below the tab bar, and is hidden on the Advanced tab, where the name is already editable, and on the Notes tab.

## [75] - 2026-08-12

### Fixed

- Group chevrons are now hidden when expanding a group would reveal no additional members
- Context-menu action now reads "Expand all"

## [74] - 2026-08-12

### Changed

- The sidebar now opens only via a card's information icon or the new **Information** context-menu entry; clicking a card just selects it

### Fixed

- The sidebar no longer lists the Data Objects of the previously selected LNode

## [73] - 2026-08-11

### Fixed

- Notes can now contain up to 8192 characters.

## [72] - 2026-07-27

### Added

- Instantiate or Update a Function (FSD) or Application (ASD) via the merge review: right-click a node → **Instantiate**/**Update** → Function/Application → pick a `.fsd`/`.asd` file → review the diff tree (accept/skip each change, edit allowed attributes, inspect the source/target XML) → apply atomically, or cancel with no side effects. For ASD, the Application is named `<name>_App`, its root Function `<App>_Fn`, and both are wired into a `FunctionRole` under the matching `FunctionCategory`; re-running **Update** reconciles an existing instance against a changed template without spurious name-only diffs.

## [71] - 2026-07-24

### Fixed

- Right-clicking grouped elements now opens their context menu
- Newly created elements now appear in the structure without reloading

## [69] - 2026-07-16

### Added

- unwrap Private children elements
- Grouping now forms as soon as one collapsed element sits among 2+ siblings, so folding/unfolding is gradual
- Keyboard navigation and screen-reader accessibility for the structure tree: arrow keys move between siblings (left/right), to the parent (up) and into children (down); Enter/Space expand or collapse; Home/End jump to the first/last sibling
- Groups can be collapsed and expanded, showing a compact, scrollable preview of their members when collapsed
- Every card — including grouped members — shows an info button (replacing the expand/collapse chevron) that opens the element in the side panel
- Element titles are shown consistently on the cards and in the side panel using the standard presentation format

### Fixed

- Diagram layout: opened cards keep the order they were opened in (newest next to its group) — no more jumping
- Connections render as one clean, centered bus per parent (no more boxy or split edges)
- Cards and their sub-trees no longer overlap

## [68] - 2026-07-07

### Fixed

- Switching between Allocation Roles updates sidebar content correctly

## [67] - 2026-07-03

### Added

- Elements expand and collapses now by mouse click
- Element levels can be expanded by right click
- Expand All / Collapse All button in the small icon bar
- Mini Map can be toggled in the small icon bar (default deactivated)
- Mini Map allows drag and zoom with mouse

## [66] - 2026-06-30

### Fixed

- ASD extraction: `Function` elements and their children are now placed at their correct hierarchy level (e.g. `Bay`) instead of being incorrectly extracted at `Substation` level.
...
- Delete button of unsaved labels no longer deletes the wrong label
...
- DO dropdown no longer disappears after adding a label to a DO/DA in the sidebar

## [60] - 2026-05-19

### Changed

- Add SignalRole for DOS/DAS under direct (Sub)Function parent within Application

## [59] - 2026-05-11

### Added

- FunctionCategoryRef being created on Function/ Subfunction instantiation
